...
import com.iktakademija.Dan0401_ProjekatSpeed.entitires.CategoryEntity;
...
public class CategoryController {
	...
	// Mock databaseK 
	private List<CategoryEntity> mockDataBaseB;

	// Get mock database
	private List<CategoryEntity> getMockDataBase() {
		if (mockDataBaseB == null) {
			mockDataBaseB = new ArrayList<CategoryEntity>();
			...
			mockDataBaseB.add(c1);
			mockDataBaseB.add(c2);
			mockDataBaseB.add(c3);
		}
		return mockDataBaseB;
	}
	...
	public CategoryEntity changeCategory(@PathVariable(name = "id") Integer id, @RequestBody CategoryEntity cat) {
		...
		for (CategoryEntity ce : getMockDataBase())
